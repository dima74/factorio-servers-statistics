...
fn main() {
    ...
    let put_object_request = s3_client.put_object(PutObjectRequest {
        bucket: BUCKET.into(),
        key: BUCKET_KEY.into(),
    ...
    runtime.block_on(put_object_request).expect("could not upload");

    let get_object_request = s3_client.get_object(GetObjectRequest {
        bucket: BUCKET.into(),
        key: BUCKET_KEY.into(),
        ..Default::default()
    });
    let get_object_result = runtime.block_on(get_object_request).expect("could not download");
