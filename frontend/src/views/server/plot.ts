...
export default class Plot {
  ...
  private extractIntervals(games: Game[]): PlayerInterval[] {
    return games.flatMap(game =>
        game.playersIntervals.filter(
            playerInterval => !(
                playerInterval.begin >= this.end
                || playerInterval.end && playerInterval.end <= this.begin
            ),
        ),
    );
  }
  ...
  buildPlot() {
    ...
    for (const playerInterval of this.playersIntervals) {
      ...
      if (!Number.isInteger(partBegin)) {
        ...
        if (this.partsPlayerList[partBeginInteger].find(playerInterval2 => playerInterval2.name == playerInterval.name) === undefined) {
          this.partsPlayerList[partBeginInteger].push(playerInterval);
        }
